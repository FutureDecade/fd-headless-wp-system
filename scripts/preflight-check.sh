...
FD_DEBUG_CONSOLE_ENABLED="${FD_DEBUG_CONSOLE_ENABLED:-false}"
...
check_allowed_value "FD_DEBUG_CONSOLE_ENABLED" "true" "false"

if [[ "${FD_DEBUG_CONSOLE_ENABLED}" == "true" && -z "${FD_DEBUG_TOKEN:-}" ]]; then
  add_config_error "FD_DEBUG_CONSOLE_ENABLED=true 时必须设置 FD_DEBUG_TOKEN"
fi
...
FD_THEME_RELEASE_TAG="${FD_THEME_RELEASE_TAG:-v1.1.4}"
FD_PAGE_COMPOSER_RELEASE_TAG="${FD_PAGE_COMPOSER_RELEASE_TAG:-v0.2.10}"
FD_ADMIN_UI_RELEASE_TAG="${FD_ADMIN_UI_RELEASE_TAG:-v1.3.4}"
FD_MEMBER_RELEASE_TAG="${FD_MEMBER_RELEASE_TAG:-v1.0.6}"
FD_PAYMENT_RELEASE_TAG="${FD_PAYMENT_RELEASE_TAG:-v1.0.3}"
FD_COMMERCE_RELEASE_TAG="${FD_COMMERCE_RELEASE_TAG:-v1.0.0}"
FD_CONTENT_TYPES_RELEASE_TAG="${FD_CONTENT_TYPES_RELEASE_TAG:-v0.5.1}"
FD_FORMS_RELEASE_TAG="${FD_FORMS_RELEASE_TAG:-v0.1.0}"
FD_AI_ROUTER_RELEASE_TAG="${FD_AI_ROUTER_RELEASE_TAG:-v2.2.5}"
FD_WEBSOCKET_PUSH_RELEASE_TAG="${FD_WEBSOCKET_PUSH_RELEASE_TAG:-v1.0.12}"
...
need_wordpress_asset_fetch() {
  ...
  if [[ ! -f "${ROOT_DIR}/runtime/wp-content/plugins/fd-forms/fd-forms.php" ]]; then
    return 0
  fi
