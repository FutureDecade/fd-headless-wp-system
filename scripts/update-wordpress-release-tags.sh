...
if [[ -z "${NEW_FD_THEME_RELEASE_TAG}" && -z "${NEW_FD_MEMBER_RELEASE_TAG}" && -z "${NEW_FD_PAYMENT_RELEASE_TAG}" && -z "${NEW_FD_COMMERCE_RELEASE_TAG}" ]]; then
...
grep -E '^FD_(THEME|MEMBER|PAYMENT|COMMERCE)_RELEASE_TAG=' "${ENV_FILE}"
