...
- GitHub Actions 里的 `Deploy Test Server` 现在既支持手动填写 release tag，也支持被 `repository_dispatch` 传入新的 `FRONTEND_IMAGE` / `WEBSOCKET_IMAGE`；是否立即更新由服务器 `.env` 里的 `FD_RUNTIME_IMAGE_UPDATE_POLICY` 决定，纯前端运行时镜像更新会走轻量路径
